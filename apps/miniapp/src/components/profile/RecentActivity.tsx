      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-3">
        ...
        <div className={`space-y-2 ${showAll ? 'max-h-none' : 'max-h-40'} ${showAll ? '' : 'overflow-y-auto'} relative`}>
          ...
          {displayedActivities.map((activity, index) => {
            ...
            return (
              <motion.div
                key={activity.id}
                className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-white/5"
                style={{ margin: '2px' }}
        ...
        {activities.length > 3 && (
          <motion.button
            ...
            className="w-full mt-3 flex items-center justify-center gap-2 text-xs text-gray-600 dark:text-white/60 hover:text-gray-800 dark:hover:text-white/80 hover:bg-gray-100 dark:hover:bg-white/5 py-2 px-3 rounded-md border border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20"
